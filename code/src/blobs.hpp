...
inline cvBlob initBlob(int id, Rect rectangle)
{
	cvBlob B = { id,rectangle.x,rectangle.y,rectangle.width,rectangle.height,UNKNOWN};
	return B;
}
...
void checkNeighbors(Mat aux_in,Mat aux_out,int i,int j,int ID,Mat& aux_in2,Mat& aux_out2);
